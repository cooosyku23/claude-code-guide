...
> **注**: 定義項目の反映は公式ドキュメント「Use subagent definitions for teammates」に基づきます（2026年5月22日時点で確認）。同ドキュメントは、チームメイトがサブエージェント定義の `tools` allowlist と `model` を honor し、定義本文をシステムプロンプトへ追記する（置換ではなく追記）と明記しています。Task ツールが付与されない点は実テストに基づきます（2026年2月時点）。Research Preview のため、今後の更新で挙動が変わる可能性があります。
...
- ✅ **チームメイトの階層的委任不可**: チームメイトとして起動したエージェントには Task ツールが付与されないため、さらにサブエージェントを呼ぶことはできない（実測、2026年2月時点）
