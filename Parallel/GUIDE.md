...
本ガイドは Git worktree を直接扱う流儀を中心に解説します。Claude Code 公式の worktree サポート（`claude --worktree` フラグ、`.worktreeinclude` など）については「Claude Code の worktree サポート（公式機能）」節に別途まとめています。最新照合 2026年5月24日時点。Claude Code の関連機能は更新されることがあるため、最新の挙動は公式ドキュメントを併せて確認してください。
...
3. [Claude Code の worktree サポート（公式機能）](#claude-code-の-worktree-サポート公式機能)
4. [Git worktreeの基本操作](#git-worktreeの基本操作)
5. [競合を防ぐ設計](#競合を防ぐ設計)
6. [実践例：Todoアプリを並列開発する](#実践例todoアプリを並列開発する)
7. [Claudeへの指示テンプレート集](#claudeへの指示テンプレート集)
8. [トラブルシューティング](#トラブルシューティング)
9. [チーム開発での活用](#チーム開発での活用)
10. [補足：セッション管理とCLAUDE.md](#補足セッション管理とclaudemd)
11. [参考リンク](#参考リンク)
...
## Claude Code の worktree サポート（公式機能）

Claude Code には worktree を組み込みで扱う公式機能があります（公式 [Worktrees ドキュメント](https://code.claude.com/docs/en/worktrees) 2026年5月24日時点）。素朴な `git worktree add` を直接叩く前に、次のいずれかで済まないかを先に検討してください。本ガイド以下の手動操作は、これらが使えない環境か、worktree の内部挙動・競合防止の設計原則を学びたい場合に参照してください。

### `claude --worktree` で1コマンドで起動する

`claude --worktree <名前>`（または `-w <名前>`）で、worktree を作成してそのままその中で Claude Code セッションを起動できます。worktree は既定でリポジトリルートの `.claude/worktrees/<名前>/` に作られ、`worktree-<名前>` という新規ブランチが切られます。

```bash
# feature-auth という worktree を作って Claude を起動
claude --worktree feature-auth

# 別ターミナルで2つ目を起動
claude --worktree bugfix-123

# 名前を省略するとランダム名（例 bright-running-fox）が生成される
claude --worktree
```

PR 単位で worktree を切ることもできます（`origin` から `pull/<番号>/head` を fetch して `.claude/worktrees/pr-<番号>/` を作成）。

```bash
claude --worktree "#1234"
```

セッション中に「worktree で作業して」と Claude に依頼すれば、`EnterWorktree` ツールで同等の操作が行われます。

### `.claude/worktrees/` を `.gitignore` に追加する

公式ドキュメントは「`.claude/worktrees/` を `.gitignore` に追加すること」を Tip として明記しています。これを忘れると、メインの作業ツリーで worktree 配下のファイルがすべて untracked として現れます。本リポジトリの `.gitignore` も同行を含めてあります。

### `.worktreeinclude` で gitignored なローカル設定を持ち込む

worktree は素のチェックアウトなので、メインリポジトリの `.env` や `.env.local` など gitignored なファイルは自動では持ち込まれません。プロジェクトルートに `.worktreeinclude` ファイルを置くと、そこに列挙したファイル（`.gitignore` 構文）のうち gitignored なものだけを、worktree 作成時にコピーしてくれます。トラックされているファイルは重複コピーされません。
...
以降の節は、`git worktree` を直接扱う流儀を、競合防止の設計原則・実践例・トラブルシュート付きで解説します。`claude --worktree` 等の公式機能を使う場合でも、内部で起きている worktree 操作・命名・片付けは共通なので、本ガイドの基礎知識は引き続き有用です。
...
```bash
# 観察専用worktreeを作成（--detach で main の内容を参照。ブランチは作らない）
git worktree add --detach ../project-watch main
```
...
git worktree add --detach ../todo-app-watch main
```
...
# 4. new-feature 用のworktreeを作成して作業を続ける
#    （mainのworktreeはmainのまま。new-featureはこのworktree側で使う）
cd ..
