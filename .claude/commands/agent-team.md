...
`.claude/examples/` ディレクトリにサンプルプロンプトが存在する場合は、それらを Read して出力スタイルや構成の参考にしてください。

1. `.claude/examples/README.md` を Read し、サンプルの位置づけと運用方針（良いプロンプトをここに保存し直すことで以後の出力品質を安定させる、など）を把握する。
2. `ls .claude/examples/` で利用可能なサンプルを確認する。
3. 今回のタスク性質（Step 2a の分類）に最も近いサンプルを最低1ファイル Read し、章立て・テンプレートの粒度・制約節の書き方を参考にする。

`README.md` 以外のサンプルが存在しない場合は、2・3 をスキップしてください。サンプルはあくまで参考であり、タスクに応じた最適な設計を優先してください。タスク性質と著しく異なるサンプルにスタイルを引きずられないよう、参照対象は「最も近いもの」に絞ってください。
